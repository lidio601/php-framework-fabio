...
class JDatabaseSQLLite extends JDatabase
{
	/**
	...
	*/
	...
	function __destruct()
	{
		$return = false;
		if ($this->_resource) {
			$return = sqlite_close($this->_resource);
	...
	function query()
	{
		...
		if (!$this->_cursor)
		{
			...
			$this->_errorNum = $this->dbo->lastErrorCode();
			//$this->_errorMsg = sqlite_error_string( $this->_errorNum )." SQL=$sql";
			$this->_errorMsg = $this->dbo->lastErrorMsg();
	...
	{
		return count($this->_cursor->fetchArray());
	...
	function loadResult()
	{
		...
		return $ret;
		*/
		return $this->dbo->querySingle($this->_sql);
	}
	...
	*/
	function loadAssoc()
	{
		...
		return $ret;
		*/
		return $this->dbo->querySingle($this->_sql,true);
	}
